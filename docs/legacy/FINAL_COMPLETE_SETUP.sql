/**
 * ============================================
 * GOALSQUAD - FINAL COMPLETE DATABASE SETUP
 * ============================================
 * 
 * ALLT I EN FIL - KOMPLETT SYSTEM
 * 
 * Inkluderar:
 * - Core tables (profiles, merchants, products, orders)
 * - Communities & members
 * - Messaging system (conversations, broadcasts)
 * - MOQ & warehouses
 * - Product flow tracking
 * - Security (guardian approvals, audit logs)
 * - RLS policies
 * - Functions
 * - Sample data
 * 
...
CREATE INDEX IF NOT EXISTS idx_guardian_approvals_guardian ON guardian_action_approvals(guardian_id);
CREATE INDEX IF NOT EXISTS idx_guardian_approvals_seller ON guardian_action_approvals(seller_id);
CREATE INDEX IF NOT EXISTS idx_guardian_approvals_status ON guardian_action_approvals(status);
...
ALTER TABLE guardian_action_approvals ENABLE ROW LEVEL SECURITY;
...
-- Guardian Approvals
DO $$ BEGIN CREATE POLICY "guardian_approvals_select_own" ON guardian_action_approvals FOR SELECT USING (
  guardian_id = auth.uid() OR seller_id = auth.uid()
);
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN CREATE POLICY "guardian_approvals_service_role_all" ON guardian_action_approvals FOR ALL USING (auth.jwt() ->> 'role' = 'service_role');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
-- 
-- 21 TABELLER SKAPADE:
-- ✅ Core: profiles, merchants, products, communities, community_members, orders, order_items
-- ✅ Warehouses: consolidation_warehouses, warehouse_inventory, merchant_shipments, merchant_shipment_items, warehouse_allocations, pending_moq_orders, shipments
-- ✅ Messaging: conversations, conversation_participants, messages, message_reads, broadcast_messages, broadcast_recipients
-- ✅ Security: guardian_action_approvals
--
-- SAMPLE DATA:
-- ✅ 5 Merchants
-- ✅ 11 Products
...
-- 2. Sätt environment variables i Vercel
-- 3. Redeploy
-- 4. Test live site!
--
-- ============================================

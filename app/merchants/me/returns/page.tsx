            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              <div>
                ...
                {selectedReturn.shipped_at && (
                  <p className="text-sm text-gray-600">Skickad: {new Date(selectedReturn.shipped_at).toLocaleString('sv-SE')}</p>
                )}
                {selectedReturn.received_at && (
                  <p className="text-sm text-gray-600">Mottagen: {new Date(selectedReturn.received_at).toLocaleString('sv-SE')}</p>
                )}
              </div>
              <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                {selectedReturn.status === 'pending' && (
                  <>
                    ...
                    <button
                      onClick={() => {
                        const reason = prompt('Ange avslagsorsak:');
                        if (reason) updateReturnStatus(selectedReturn.id, 'rejected', reason);
                      }}
                ...
                {selectedReturn.status === 'approved' && (
                  <button
                    onClick={() => updateReturnStatus(selectedReturn.id, 'in_transit')}
    ...
    <div className="min-h-screen bg-gray-50 py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-gray-600">Hantera returer från dina kunder</p>
        </div>

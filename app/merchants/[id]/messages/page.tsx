...
interface Conversation {
  id: string;
  name: string;
  entity_type: 'community' | 'club' | 'class';
  entity_id: string;
  last_message: string;
...
export default function MerchantMessages() {
  ...
  const fetchConversations = async () => {
    try {
      const response = await fetch(`/api/merchants/${merchantId}/conversations`);
  ...
  const filteredConversations = conversations.filter(conv => {
    ...
    const matchesFilter = filter === 'all' || conv.entity_type === filter;
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        >
          ...
          <p className="text-gray-600">Kommunicera med föreningar, klubbar och klasser</p>
        </motion.div>
        ...
        >
          <div className="flex flex-col md:flex-row gap-4">
            <div className="flex-1 relative">
              ...
              <input
                type="text"
                placeholder="Sök efter konversation..."
                value={search}
            ...
            >
              ...
              <option value="community">Föreningar</option>
              <option value="club">Klubbar</option>
              <option value="class">Klasser</option>
            </select>
          ...
          >
            {filteredConversations.length === 0 ? (
              <div className="bg-white rounded-2xl shadow-lg p-12 text-center">
                ...
                <p className="text-gray-600">Börja kommunicera med föreningar, klubbar och klasser</p>
              </div>
            ) : (
                ...
                >
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-gray-600 mb-2">{getEntityTypeLabel(conv.entity_type)}</p>
            ...
            {selectedConversation ? (
              <div className="bg-white rounded-2xl shadow-lg h-[600px] flex flex-col">
                ...
                <div className="p-4 border-b border-gray-200">
                  ...
                  <p className="text-sm text-gray-600">{getEntityTypeLabel(selectedConversation.entity_type)}</p>
                </div>
                  ...
                  {messages.length === 0 ? (
                    ...
                    messages.map((message, index) => (
                      <motion.div
                        ...
                        animate={{ opacity: 1, y: 0 }}
                        transition={{ delay: index * 0.05 }}
                        className={`flex ${message.is_from_merchant ? 'justify-end' : 'justify-start'}`}
                      >
                        <div className={`max-w-[70%] rounded-2xl p-4 ${
                          message.is_from_merchant
                          ...
                          <p className={`text-xs mt-2 ${
                            message.is_from_merchant ? 'text-primary-200' : 'text-gray-500'
                          }`}>
